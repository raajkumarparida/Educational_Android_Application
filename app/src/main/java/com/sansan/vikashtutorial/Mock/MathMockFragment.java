...
public class MathMockFragment extends Fragment {
...
//    private SearchBar searchBar;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_math_mock, container, false);
...
//        });
    ...
    private void setupHeaderClickListener(TextView headerView, String classLevel, String header) {
        headerView.setOnClickListener(view -> {
            ...
            editor.putString("selectedCategory", classLevel + "MathMock");
        ...
        FirebaseFirestore.getInstance().collection("users").document(userId).get()
                .addOnSuccessListener(documentSnapshot -> {
                    if (documentSnapshot.exists()) {
                        ...
                        if (categoryPath != null) {
                            if (categoryPath.equals("11thMathMock")) {
                                fetchCHSEData();
                                fetchCBSEData();
                                fetchJEEData();
                                fetchNEETData();
                                fetchOUATData();
                            } else if (categoryPath.equals("12thMathMock")) {
                                fetch12thCHSEData();
                                fetch12thCBSEData();
                                fetch12thJEEData();
                                fetch12thNEETData();
    ...
    private void fetchCHSEData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("11thMathMock/CHSE");
        fetchData(mockRef, itemListCHSE, adapterCHSE, recyclerViewCHSE, noListMessageCHSE, chseLinear);
    }

    private void fetchCBSEData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("11thMathMock/CBSE");
        fetchData(mockRef, itemListCBSE, adapterCBSE, recyclerViewCBSE, noListMessageCBSE, cbseLinear);
    }

    private void fetchJEEData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("11thMathMock/JEE");
        fetchData(mockRef, itemListJEE, adapterJEE, recyclerViewJEE, noListMessageJEE, jeeLinear);
    }

    private void fetchNEETData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("11thMathMock/NEET");
        fetchData(mockRef, itemListNEET, adapterNEET, recyclerViewNEET, noListMessageNEET, neetLinear);
    }

    private void fetchOUATData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("11thMathMock/OUAT");
        fetchData(mockRef, itemListOUAT, adapterOUAT, recyclerViewOUAT, noListMessageOUAT, ouatLinear);
    }

    private void fetch12thCHSEData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("12thMathMock/CHSE");
        fetchData(mockRef, itemListCHSE, adapterCHSE, recyclerViewCHSE, noListMessageCHSE, chseLinear);
    }

    private void fetch12thCBSEData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("12thMathMock/CBSE");
        fetchData(mockRef, itemListCBSE, adapterCBSE, recyclerViewCBSE, noListMessageCBSE, cbseLinear);
    }

    private void fetch12thJEEData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("12thMathMock/JEE");
        fetchData(mockRef, itemListJEE, adapterJEE, recyclerViewJEE, noListMessageJEE, jeeLinear);
    }

    private void fetch12thNEETData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("12thMathMock/NEET");
        fetchData(mockRef, itemListNEET, adapterNEET, recyclerViewNEET, noListMessageNEET, neetLinear);
    }

    private void fetch12thOUATData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("12thMathMock/OUAT");
        fetchData(mockRef, itemListOUAT, adapterOUAT, recyclerViewOUAT, noListMessageOUAT, ouatLinear);
    }
