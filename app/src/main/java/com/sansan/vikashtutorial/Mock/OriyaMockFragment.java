...
public class OriyaMockFragment extends Fragment {
...
//    private SearchBar searchBar;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_oriya_mock, container, false);
...
//////                    if (document.getId().equals("class11thPdf")) {
...
//        });
    ...
    private void setupHeaderClickListener(TextView headerView, String classLevel, String header) {
        headerView.setOnClickListener(view -> {
            ...
            editor.putString("selectedCategory", classLevel + "OriyaMock");
        ...
        FirebaseFirestore.getInstance().collection("users").document(userId).get()
                .addOnSuccessListener(documentSnapshot -> {
                    if (documentSnapshot.exists()) {
                        ...
                        if (categoryPath != null) {
                            if (categoryPath.equals("11thOriyaMock")) {
                                fetchCHSEData();
                                fetchCBSEData();
                                fetchJEEData();
                                fetchNEETData();
                                fetchOUATData();
                            } else if (categoryPath.equals("12thOriyaMock")) {
                                fetch12thCHSEData();
                                fetch12thCBSEData();
                                fetch12thJEEData();
                                fetch12thNEETData();
    ...
    private void fetchCHSEData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("11thOriyaMock/CHSE");
        fetchData(mockRef, itemListCHSE, adapterCHSE, recyclerViewCHSE, noListMessageCHSE, chseLinear);
    }

    private void fetchCBSEData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("11thOriyaMock/CBSE");
        fetchData(mockRef, itemListCBSE, adapterCBSE, recyclerViewCBSE, noListMessageCBSE, cbseLinear);
    }

    private void fetchJEEData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("11thOriyaMock/JEE");
        fetchData(mockRef, itemListJEE, adapterJEE, recyclerViewJEE, noListMessageJEE, jeeLinear);
    }

    private void fetchNEETData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("11thOriyaMock/NEET");
        fetchData(mockRef, itemListNEET, adapterNEET, recyclerViewNEET, noListMessageNEET, neetLinear);
    }

    private void fetchOUATData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("11thOriyaMock/OUAT");
        fetchData(mockRef, itemListOUAT, adapterOUAT, recyclerViewOUAT, noListMessageOUAT, ouatLinear);
    }

    private void fetch12thCHSEData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("12thOriyaMock/CHSE");
        fetchData(mockRef, itemListCHSE, adapterCHSE, recyclerViewCHSE, noListMessageCHSE, chseLinear);
    }

    private void fetch12thCBSEData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("12thOriyaMock/CBSE");
        fetchData(mockRef, itemListCBSE, adapterCBSE, recyclerViewCBSE, noListMessageCBSE, cbseLinear);
    }

    private void fetch12thJEEData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("12thOriyaMock/JEE");
        fetchData(mockRef, itemListJEE, adapterJEE, recyclerViewJEE, noListMessageJEE, jeeLinear);
    }

    private void fetch12thNEETData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("12thOriyaMock/NEET");
        fetchData(mockRef, itemListNEET, adapterNEET, recyclerViewNEET, noListMessageNEET, neetLinear);
    }

    private void fetch12thOUATData() {
        DatabaseReference mockRef = FirebaseDatabase.getInstance().getReference("12thOriyaMock/OUAT");
        fetchData(mockRef, itemListOUAT, adapterOUAT, recyclerViewOUAT, noListMessageOUAT, ouatLinear);
    }
